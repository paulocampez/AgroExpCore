...
///       Must update database connection in appsettings.json - "AgroExpCoreDB".  
    ...
    public class TokenTest : BaseTest
    {
        ...
        public static async Task token_get(HttpClient client)
        {
            ...
            { //IS4
                ...
                //var response1 = await tokenClient1.RequestClientCredentialsAsync("AgroExpCore");
                //var resp1 = response1.Json;


                //BAD client test
                var response = LoginUsingIdentityServer(is4token, "AgroExpCore-BAD", "secret", "read-write", "my@email.com", "mysecretpassword123");
                ...
                //BAD grant test
                response = LoginUsingIdentityServer(is4token, "AgroExpCoreClient", "secret", "read-write", "my@email.com", "mysecretpassword123-BAD");
                ...
                response = LoginUsingIdentityServer(is4token, "AgroExpCoreClient", "secret", "read-write", "my@email.com", "mysecretpassword123");
                response_json = response.AccessToken;
                if (response.IsError)
                {
                    Console.WriteLine(response.Error);
